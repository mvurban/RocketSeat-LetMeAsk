...
import { useLike } from "../../domain/Like";
...
export function Room(){
   ...
   async function handleLikeQuestion(idQuestion : string ){
                  ...
                  questions.map((obj, index)=>{
                     return (
                        <Question key={obj.id} question={obj}>
                           <button onClick={()=>handleLikeQuestion(obj.id)} className="like-button">
                              <span>{obj.likes?.length}</span>
                              <svg className={obj.isLikedByCurrentUser ? "liked" : "" } width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
